...
import cn.cathead.ai.domain.model.service.form.loading.FormLoading;
...
public class DynamicFormImpl implements IDynamicForm {
    ...
    public FormConfiguration getFormConfiguration(String provider, String type) {
        ...
        // 委托给配置管理器
        return formLoading.getFormConfiguration(provider, type);
    }
    ...
    public ValidationResult validateFormData(String provider, String type, Map<String, Object> formData) {
        ...
        if (config == null) {
            ...
            return result;
                ...
                // 委托给校验器进行校验
        return formValidator.validateFormData(config, formData);
    }
     ...
     */
    @Override
    public Map<String, Object> applyDefaultValues(String provider, String type, Map<String, Object> formData) {
        log.debug("应用默认值，provider: {}, type: {}", provider, type);
        
        // 获取表单配置
        FormConfiguration config = getFormConfiguration(provider, type);
        if (config == null) {
            log.warn("未找到表单配置，无法应用默认值，provider: {}, type: {}", provider, type);
            return formData;
        }
        
        // 委托给校验器应用默认值
        return formValidator.applyDefaultValues(config, formData);
    }
    ...
    public String submitForm(String provider, String type, Map<String, Object> formData) {
        ...
        // 先应用默认值
        Map<String, Object> formDataWithDefaults = applyDefaultValues(provider, type, formData);
        log.debug("应用默认值后的表单数据: {}", formDataWithDefaults);

        ValidationResult validationResult = formValidator.validateFormData(config, formDataWithDefaults);
    ...
    /**
     * 构建Chat模型DTO
     * 注意：传入的formData已经通过applyDefaultValues处理过，包含默认值，无需再次处理默认值
     */
