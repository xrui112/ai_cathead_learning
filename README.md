...
基于领域驱动设计(DDD)架构的现代化AI学习平台，集成多模型供应商管理、模型控制协议(MCP)、检索增强生成(RAG)和智能代理(Agent)等核心功能。项目采用整洁架构原则，实现高内聚、低耦合的模块化设计。

## 🏗️ 架构概览
ai-rag-catheadLearning/
├── cathead-sy-api/ # 应用接口层
├── cathead-sy-app/ # 应用服务层
├── cathead-sy-domain/ # 领域核心层
├── cathead-sy-infrastructure/ # 基础设施层
├── cathead-sy-trigger/ # 触发器层(Controller)
└── cathead-sy-types/ # 类型定义层
